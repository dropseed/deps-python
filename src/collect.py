...
def collect(input_path, output_path):
    ...
    manifests = load_dependency_files(input_path)  # potentially recursive collection exposed as list
    ...
    for manifest in manifests:
        ...
        direct_deps.extend([dep for dep in manifest_dependencies["current"]["dependencies"].keys()])
    ...
    for lockfile in lockfiles:
        ...
        current_dependencies = lockfile.get_dependencies(direct_dependencies=direct_deps)
        ...
        if current_fingerprint != lockfile.fingerprint():
            output['lockfiles'][lockfile.filename]['updated'] = {
                ...
                'dependencies': lockfile.get_dependencies(direct_dependencies=direct_deps),
            }
